...
Public demo promotion is checked by `npm run launch:preflight` after manual safety review is recorded. The command fails until a completed reviewer session, reviewed materials, synthetic examples, sanitized notes, no critical issues open, high issues accepted or closed, manual safety review complete, public demo readiness, owner, and decision date are recorded.
