...
Public demo promotion is checked by `npm run launch:preflight` after manual safety review is recorded. The command fails until a completed reviewer session, reviewed materials, synthetic examples, sanitized review answers, sanitized finding ID, severity, area, summary, evidence, recommended change, sanitized notes, `public_issue_safe: yes` findings, no critical issues open, high issues accepted or closed, manual safety review complete, public demo readiness, owner, and decision date are recorded.
