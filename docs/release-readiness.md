...
This launch gate must pass before public demo promotion. It checks completed session fields, reviewed materials, synthetic examples, sanitized notes, issue status, owner, and decision date. It is separate from `npm run check` because automated CI should stay green while partner review is still pending.
