...
const injectionPatterns = [
  ...
  /\b(?:do not|don't|never) request human review\b/i,
  ...
  /\b(?:handle|finish|complete|review|resolve|do) (?:this|the case|the letter|the notice|the appeal|the denial)\b.{0,80}\bwithout (?:a )?qualified helper\b/i,
  ...
  /\byou will receive (?:fema|disaster|financial|rental|repair|lodging|housing|assistance|payment)\b/i,
  /\bapproval is certain\b/i,
  /(?:^|[\n.!?]\s*)(?:this )?denial will be overturned\b/i,
  /(?:^|[\n.!?]\s*)(?:the )?appeal will succeed\b/i,
  /\bfema owes you\b/i,
  /\b(?:openrelief|we|this app|this tool|the tool|our team) (?:will |can )?(?:submit|file|send|transmit|forward|deliver)s? your .*(?:appeal|application|claim|to fema|to the agency)\b/i,
  /\b(?:openrelief|we|this app|this tool|the tool|our team) (?:has )?(?:submitted|filed|sent|transmitted|forwarded|delivered) your .*(?:appeal|application|claim|to fema|to the agency)\b/i,
  /\byour .*(?:appeal|application|claim) (?:is|was|will be) (?:sent|delivered|filed|submitted)(?: .*?)? by (?:openrelief|our team|this app|this tool|the tool)\b/i,
  /\bopenrelief handles filing your .*(?:appeal|application|claim)\b/i,
  /(?<!do not )(?<!don't )(?<!never )\bset (?:the )?(?:fema|official|agency) (?:letter|notice|request) aside\b/i,
  /(?<!do not )(?<!don't )(?<!never )\bignore (?:the )?(?:fema|official|agency) deadline\b/i,
  ...
  /(?<!do not )(?<!don't )(?<!never )\bfile in (?:federal|state) court\b/i,
  /\bdemand damages from (?:fema|the agency)\b/i,
  /(?<!do not )(?<!don't )(?<!never )\bthreaten\b.{0,80}\bwith legal action\b/i,
...
const restrictedIdentifierPatterns = [
  ...
  {
    pattern:
      /\b(?:(?:proof\s+of\s+(?:occupancy|ownership)|occupancy\s+proof|utility\s+bills?|mortgage(?:\s+(?:documents?|documentation|records?|statements?))?|deed(?:\s+(?:records?|statements?))?|deeds?\s+of\s+trust|deed\s+or\s+title)|(?:property\s+tax|tax\s+assessment|escrow)\s+(?:statements?|records?|receipts?|bills?|analysis|analyses)|tax\s+bills?|homeowners?(?:'s)?\s+insurance\s+statements?|(?:real\s+property|structural)\s+insurance\s+(?:documents?|bills?|payment\s+records?)|(?:occupancy|residence|ownership|lease|utility|title)\s+records?)\s*(?:(?:id|number|no\.?)\s*)?(?:[:#-]\s*)*(?=[A-Z0-9-]*\d)[A-Z0-9][A-Z0-9-]{5,}\b/gi,
  ...
  {
    pattern:
      /\b(?:voter\s+registration\s+cards?|social\s+service\s+organization\s+documents?|(?:federal|state|federal\s+or\s+state)\s+benefit\s+documents?|mobile\s+home\s+park\s+(?:documents?|letters?|owners?|managers?)|vehicle\s+registrations?|affidavits?\s+of\s+residency|court\s+(?:documentation|documents?)|school\s+(?:documents?|records?))\s*(?:(?:id|number|no\.?)\s*)?(?:[:#-]\s*)*(?=[A-Z0-9-]*\d)[A-Z0-9][A-Z0-9-]{5,}\b/gi,
    replacement: "[residence evidence identifier removed]"
  },
  {
    pattern:
  ...
  {
    pattern:
      /\b(?:(?:medical|medication|transportation)\s+(?:receipts?|notes?)|medical,\s+medication,?\s+or\s+transportation\s+receipts?|receipts?\s+for\s+transportation(?:\s+and\s+temporary\s+lodging)?|(?:temporary\s+lodging|evacuation\s+lodging|lodging\s+expense|short-term\s+lodging|lodging|hotel|motel)\s+(?:receipts?|records?|notes?))\s*(?:(?:id|number|no\.?)\s*)?(?:[:#-]\s*)*(?=[A-Z0-9-]*\d)[A-Z0-9][A-Z0-9-]{5,}\b/gi,
...
export const analyzeLetter = (letterText: string): LetterAnalysis => {
  ...
  if (hasRequestForInformationCue(normalized)) {
    return {
      ...
      summary: withDeadlineSummary("This letter asks for more details before FEMA can decide."),
      facts,
      uncertainties,
      detectedDeadlines,
      detectedRequests,
  ...
  if (hasDeniedApplicationCue(normalized)) {
    return {
      letterType: "denial",
      summary: withDeadlineSummary("This letter denies the request. Ask a human helper before next steps."),
      facts,
      uncertainties,
      detectedDeadlines,
      detectedRequests,
  ...
  if (normalized.includes("inspection") || normalized.includes("inspector")) {
    return {
      ...
      summary: withDeadlineSummary("This letter is about an inspection step."),
      facts,
      uncertainties,
      detectedDeadlines,
      detectedRequests,
  ...
  if (detectedDeadlines.length > 0 || normalized.includes("deadline")) {
    return {
      ...
      summary: withDeadlineSummary("This letter has a due date."),
      facts,
      uncertainties,
      detectedDeadlines,
      detectedRequests,
  ...
  if (normalized.includes("approved") || normalized.includes("approval")) {
    return {
      letterType: "approval",
      summary: withDeadlineSummary("This letter approves assistance. Review amounts, dates, and next steps."),
      facts,
      uncertainties,
      detectedDeadlines,
      detectedRequests,
  ...
  return {
    letterType: "unknown",
    summary: withDeadlineSummary("This letter needs human review because the type is unclear."),
    facts,
    uncertainties,
    detectedDeadlines,
    detectedRequests,
...
export const detectRiskFlags = (intakeText: string, letter?: LetterAnalysis): RiskFlag[] => {
  ...
  if (/homeless|no place to stay|nowhere to stay|displaced|sleeping outside|staying outside|sleeping in (?:a |our |my )?(?:car|truck|van|vehicle|rv|tent)|staying in (?:a |our |my )?tent|living in (?:a |our |my )?(?:car|truck|van|vehicle|rv|tent)|couch surfing|shelter|evict/i.test(normalized)) {
  ...
  if (
    /immigration|undocumented|deportation|visa|mixed status|asylum|alien registration number|a-number|anumber|a\s*#|uscis(?:\s+number|\s*#)?|green card|lawful permanent resident|temporary protected status|\btps\b/i.test(
      normalized
    )
  ) {
