...
export const detectRiskFlags = (intakeText: string, letter?: LetterAnalysis): RiskFlag[] => {
  ...
  if (letter?.letterType === "denial" || letter?.detectedDeadlines.length || /appeal deadline|response deadline|deadline is (?:today|tomorrow)|need to appeal|appeal by|denied assistance/i.test(normalized)) {
  ...
  if (/disability|wheelchair|walker|cane|accessible|accommodation|service animal|asl interpreter/i.test(normalized)) {
