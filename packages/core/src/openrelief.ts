...
const injectionPatterns = [
  ...
  /(?:ask for|share|provide|send|enter|include|upload|give).*(?:payment codes?|full bank numbers?|bank numbers?|bank account numbers?|routing numbers?|full ssn|ssn|social security numbers?|dob|date of birth|birth date|immigration status|visa status|full application id|fema codes?|access codes?|door codes?|entry codes?|verification codes?|passwords?|pins?|otps?|passcodes?)/i
];
...
const responseWithinDaysPattern = /\b(?:respond|reply|send|submit|provide)(?:\s+[a-z][a-z-]*){0,6}\s+within\s+\d{1,3}\s+days\b/i;
...
const restrictedIdentifierPatterns = [
  ...
  {
    pattern:
      /\b(?:dob|date of birth|birth date)\s*[:#-]?\s*(?:january|february|march|april|may|june|july|august|september|october|november|december)\s+\d{1,2},?\s+\d{4}\b/gi,
    replacement: "[date of birth removed]"
  },
  {
    pattern:
...
export const detectRiskFlags = (intakeText: string, letter?: LetterAnalysis): RiskFlag[] => {
  ...
  if (/immediate danger|in danger right now|fire.*right now|trapped|cannot evacuate|can't evacuate|cannot leave|can't leave|cannot get out|can't get out|stuck inside|smoke.*filling|breathing smoke|downed power line|live wire|power line.*sparking|sparking power line|gas leak|life[-\s]?threatening/i.test(normalized)) {
  ...
  if (/medical emergency|oxygen|medicine|medication|insulin|inhaler|prescriptions?|cpap|dialysis|urgent medical|hospital|cannot breathe|can't breathe|chest pain|heart attack|bleeding|seizure|stroke/i.test(normalized)) {
  ...
  if (/disability|wheelchair|walker|cane|blind|deaf|accessible|accommodation|service animal|asl interpreter/i.test(normalized)) {
