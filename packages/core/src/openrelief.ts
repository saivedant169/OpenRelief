...
export const detectRiskFlags = (intakeText: string, letter?: LetterAnalysis): RiskFlag[] => {
  ...
  if (
    /scam|fraud|fraudulent|fake fema|gift cards?|guarantee.*(?:fema )?approval|pay .*release (?:fema )?funds|bank account number|full bank number|routing number|ssn|social security number|door codes?|prepaid debit card|wire transfer|processing fee|application fee|expedite fee|verification fee|upfront fee|bitcoin|cryptocurrency|cash app|zelle|venmo|paypal|money order|western union|moneygram|cashier check|asked for.*(?:fema code|application id|payment code)/i.test(normalized)
  ) {
