...
const injectionPatterns = [
  ...
  /system prompt/i,
  /say .* approved/i,
  /(?:ask for|share|provide).*(?:payment codes?|full bank numbers?|bank numbers?|bank account numbers?|routing numbers?|full ssn|ssn|social security numbers?|immigration status|visa status|full application id|fema codes?|access codes?|door codes?|entry codes?|verification codes?|passwords?|pins?|otps?|passcodes?)/i
];
...
const restrictedIdentifierPatterns = [
  ...
  {
    pattern:
      /\b(?:[Ff]ull\s+[Nn]ame|[Ss]urvivor\s+[Nn]ame|[Aa]pplicant\s+[Nn]ame|[Cc]ontact\s+[Nn]ame|[Hh]ousehold\s+[Mm]ember\s+[Nn]ame|[Cc]hild\s+[Nn]ame|[Cc]ase\s+[Ww]orker\s+[Nn]ame|[Tt]enant\s+[Nn]ame|[Ll]andlord\s+[Nn]ame|[Cc]ontractor\s+[Nn]ame|[Pp]roperty\s+[Oo]wner\s+[Nn]ame)\s*[:#-]?\s*[A-Z][A-Za-z.'-]+(?:\s+[A-Z][A-Za-z.'-]+){1,3}\b/g,
