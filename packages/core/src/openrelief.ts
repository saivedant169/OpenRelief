...
export const detectRiskFlags = (intakeText: string, letter?: LetterAnalysis): RiskFlag[] => {
  ...
  if (/immediate danger|in danger right now|fire.*right now|trapped|cannot evacuate|can't evacuate|cannot leave|can't leave|cannot get out|can't get out|stuck inside|life[-\s]?threatening/i.test(normalized)) {
